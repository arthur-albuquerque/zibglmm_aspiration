---
title: "Prior Predictive Exploration - Main model"
format:
  html:
...
model1 <- cmdstan_model(here("models/stan/zibglmm_prior_model1.stan"))
```
...
prior_draws_parameters_mod <- 
  fit_prior_mod$draws(variables = c("mu", "pi", "sigma_nu"),
                           format = "df")
...
prior_draws_estimands <- 
  fit_prior_mod$draws(variables = c("p_control", "p_treatment", "RR", 
                                "OR", "RD"),
                  format = "df")
```
...
mcmc_dens(prior_draws_estimands, pars = c("p_control", "p_treatment", "RR", "OR", "RD")) +
...
n_samples <- nrow(draws)  # Adjust based on number of chains and iterations
...
```{r}
rbind(control_long, treatment_long) |> 
  ggplot(aes(x = Probability, group = Study)) +
